...
var game = {
  ...
  opGain: (ord = game.data.ord, over = game.data.over, base = game.base()) => {
    if (ord < base) {
      ...
      var tempvar = Math.floor(Math.log(ord) / Math.log(base) + 1e-20);
      var tempvar2 = base ** tempvar;
      var tempvar3 = Math.floor(ord / tempvar2 + 1e-20);
  ...
  maximize: (manmade = true) => {
    if (!manmade || game.clickCooldown === 0) {
      if (game.data.ord % game.base() === game.base() - 1 && game.data.over >= 1) {
        ...
        do {
          game.data.over -= Math.ceil((game.data.over + game.base()) / 2 - 1e-20);
  ...
  maxAll: (manmade = true) => {
    if (!manmade || game.clickCooldown === 0) {
      if (game.data.op < 2.000e230) {
        ...
        bulk = Math.floor(
          Math.log(1 + game.data.op / (100 * 2 ** game.data.incrementAuto)) / Math.log(2) + 1e-20
        );
        ...
        bulk = Math.floor(
          Math.log(1 + game.data.op / (100 * 2 ** game.data.maximizeAuto)) / Math.log(2) + 1e-20
        );
  ...
  hardy: (ord = game.data.ord, over = game.data.over, base = game.base()) => {
    var tempvar = Math.floor(ord / base + 1e-20);
    var tempvar2 = Math.floor(ord / base ** 2 + 1e-20);
  ...
  writeOrd: (color, ord = game.data.ord, over = game.data.over, base = game.base()) => {
    if (ord === 0) {
      ...
      while (remainOrd > 0 && terms < 5) {
        var power = Math.floor(Math.log(remainOrd) / Math.log(base) + 1e-20);
        if (result === ``) {
          if (power === 0) {
            ...
            if (power === 1) {
              if (Math.floor(remainOrd / base) === 1) {
                ...
                result = `${color ? `<span style="color:hsl(${game.ordColor(base, 0) * 360}, 100%, 50%)">`: ``}&omega;${Math.floor(remainOrd / base + 1e-20)}${color ? `</span>`: ``}`;
              }
            } else {
              if (Math.floor(remainOrd / base ** power) === 1) {
                ...
                result = `${color ? `<span style="color:hsl(${game.ordColor(base ** power, 0) * 360}, 100%, 50%)">`: ``}&omega;<sup>${game.writeOrd(false, power, 0)}</sup>${Math.floor(remainOrd / base ** power + 1e-20)}${color ? `</span>`: ``}`;
              }
            }
          }
        } else {
      ...
      while (remainOrd > 0) {
        var power = Math.min(41, Math.floor(Math.log(remainOrd / 1.000e230) / Math.log(3) + 1e-20));
          ...
          (x < 1000 && x % 1 !== 0) ?
            (x * 10 % 1 === 0) ?
              ...
              (x * 100 % 1 === 0) ?
                ...
                x.toFixed(3):
          x.toFixed(0):
        `${(x / 10 ** Math.floor(Math.log10(x) + 1e-20)).toFixed(3)}e${Math.floor(Math.log10(x) + 1e-20)}`,
  toggleColor: () => {
    if (game.clickCooldown === 0) {
  ...
  loop: (ms, off = false) => {
    ...
    if ((game.data.incrementAuto < 1.000e230 || game.data.maximizeAuto < 1.000e230) && (game.data.ord < 3 ** 27 || game.base() > 3)) {
      ...
      if (game.data.pendingIncrement >= 1) {
        if (game.data.pendingMaximize >= 1) {
          ...
          game.data.ord += Math.min(Math.floor(game.data.pendingIncrement + 1e-20), game.base() * Math.floor(game.data.pendingMaximize + 1e-20));

          game.data.pendingIncrement %= 1;
          game.data.pendingMaximize %= 1;
        } else if (Math.floor(game.data.pendingIncrement + 1e-20) >= game.base() - (game.data.ord % game.base())) {
          game.data.ord += game.base() - (game.data.ord % game.base()) - 1;
          game.data.over += Math.floor(game.data.pendingIncrement + 1e-20) - game.base() + (game.data.ord % game.base()) + 1;
          game.data.pendingIncrement %= 1;
        } else {
    ...
    if (game.data.pendingMaxAll >= 1 && game.data.pendingMarkup >= 1) {
      ...
      game.data.ord += Math.floor(bupCom + 1e-20) * 1.000e230;
      game.data.op += Math.floor(bupCom + 1e-20) * 1.000e230;
    }
